...
class CartController {
    public async getCart(req: Request, res: Response): Promise<void> {
        try {
            ...
            if (!cart) {
                console.log(
                    `User does not have an active cart for user id:${userId}`
                )
    ...
    public async updateCart(req: Request, res: Response): Promise<void> {
        try {
            ...
            const validatedItem: CartUpdateDto =
                await validateReceivedUpdatedCartItem(req.body)
            ...
            if (!cart) {
                ...
                if (!product) {
                    ...
                    console.warn(
                        `Product is not valid by id:${validatedItem.productId}`
                    )
                    return
                }
            }
            ...
            if (
                !cart.items.some(
                    (cartItem) =>
                        cartItem.product.id === validatedItem.productId
                )
            ) {
                res.status(400).json({
                    data: null,
                ...
                console.warn(
                    `Cart id${cart?.id} doesn't contains a product:${validatedItem.productId}`
                )
                return
                // todo or add new product to cart
            }

            const updatedCart = await updatedProductInCart(
                cart!,
                validatedItem!
            )
