...
export async function findCarByUserId(userId: string): Promise<ICartEntity | null> {
    try {
        return await CartEntity.findOne({user: userId}).populate('items.product');
...
export async function updatedProductInCart(cart: ICartEntity, cartUpdateDto: CartUpdateDto): Promise<ICartEntity | null> {
    try {
        let index;
